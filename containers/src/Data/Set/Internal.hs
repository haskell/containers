...
map f = finishB . foldl' (\b x -> insertB (f x) b) emptyB
#if __GLASGOW_HASKELL__
{-# INLINABLE map #-}
#endif
...
--
-- As long as the elements are sorted, insertB accumulates them in a Stack,
-- just as fromDistinctAscList does. On encountering an element out of order,
-- it builds a Set from the Stack and switches to using insert for all future
-- elements. This gives us O(n log n) worst case but O(n) if the elements are
-- already sorted.
--
