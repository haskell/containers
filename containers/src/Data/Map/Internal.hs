...
-- force these values.
--
-- __Note:__ This is asymptotically optimal.
-- Given @n :: Map a b, m :: Map b c@, the composition essentially maps each @a@ in @n@ to @Maybe c@, since the composed lookup yields either one
-- of the @c@ in @m@ or @Nothing@. The number of possible such mappings is \((|m| + 1) ^ {|n|}\).
-- We now follow a similar reasoning to the one for [sorting](https://en.wikipedia.org/wiki/Comparison_sort#Number_of_comparisons_required_to_sort_a_list).
-- To distinguish between @x@ possible values, we need \( \lceil \log (x) \rceil \) bits.
-- Thus, we have a lower bound of \(\log ((|m| + 1) ^{|n|}) = |n| * \log (|m| + 1)\) bits.
-- "Data.Map" lookups are comparison-based, and each comparison gives us at most one bit
-- of information: in the worst case we'll always be left with at least half
-- of the remaining possible values, meaning we need at least as many comparisons as we need bits.
--
-- @since 0.6.3.1
