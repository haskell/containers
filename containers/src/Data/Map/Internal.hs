...
iterNext :: Iterator k a -> Maybe (StrictPair (k, a) (Iterator k a))
iterNext (Iterator stk) = case stk of
  Nada -> Nothing
  Push kx x r stk' -> Just $! (kx, x) :*: Iterator (iterDown r stk')
{-# INLINE iterNext #-}
...
sameSizeLiftEq2 keq eq m1 m2 =
  ...
  where
    f kx x = EqM $ \it -> case iterNext it of
      Nothing -> False :*: it
      Just ((ky, y) :*: it') -> (keq kx ky && eq x y) :*: it'
...
liftCmp2 kcmp cmp m1 m2 = case runOrdM (foldMapWithKey f m1) (iterator m2) of
  ...
  where
    f kx x = OrdM $ \it -> case iterNext it of
      Nothing -> GT :*: it
      Just ((ky, y) :*: it') -> (kcmp kx ky <> cmp x y) :*: it'
{-# INLINE liftCmp2 #-}
